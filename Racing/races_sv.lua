    ...
    for index, race in pairs(races) do
        ...
        if source == race.owner and time < race.startTime then
            ...
            for _, player in pairs(race.players) do
                ...
                TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Race cancelled', style = { ['background-color'] = '#FF0000', ['color'] = '	#FFFFFF' } })
            end
...
AddEventHandler("StreetRaces:joinRace_sv", function(index)
    ...
    if playerMoney >= amount then
        ...
        TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'You don\'t have enough money to race!', style = { ['background-color'] = '#FF0000', ['color'] = '	#FFFFFF' } })
    end
end)
            ...
            if race.finishTime == 0 then
                ...
                for _, pSource in pairs(players) do
                    if pSource == source then
                        ...
                        TriggerClientEvent('mythic_notify:client:SendAlert', source , { type = 'inform', text = ("You won! Time:[%02d:%06.3f]"):format(timeMinutes, timeSeconds) , style = { ['background-color'] = '#FF0000', ['color'] = '	#FFFFFF' } })
...
AddEventHandler("StreetRaces:saveRace_sv", function(name, checkpoints)
    ...
    TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Track Saved', style = { ['background-color'] = '#FF0000', ['color'] = '	#FFFFFF' } })
...
AddEventHandler("StreetRaces:deleteRace_sv", function(name)
    ...
    if playerRaces[name] ~= nil then
        ...
        -- Send notification to player
        TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Track deleted', style = { ['background-color'] = '#FF0000', ['color'] = '	#FFFFFF' } })
    else
        TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'No race track found with that name.', style = { ['background-color'] = '#FF0000', ['color'] = '	#FFFFFF' } })
    end
end)
...
AddEventHandler("StreetRaces:loadRace_sv", function(name)
    ...
    if race ~= nil then
        ...
        -- Send notification to player
        TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Track loaded', style = { ['background-color'] = '#FF0000', ['color'] = '	#FFFFFF' } })
    else
        TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'No track found with that name.', style = { ['background-color'] = '#FF0000', ['color'] = '	#FFFFFF' } })
    end
end)
