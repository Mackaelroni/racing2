...
RegisterCommand("race", function(source, args)
    if args[1] == "clear" or args[1] == "leave" and ESX.PlayerData.job.name == 'racing' then
    ...
    elseif args[1] == "record" and ESX.PlayerData.job.name == 'racing' then
        -- Clear waypoint, cleanup recording and set flag to start recording
        SetWaypointOff()
        cleanupRecording()
        raceStatus.state = RACE_STATE_RECORDING
    elseif args[1] == "save" and ESX.PlayerData.job.name == 'racing' then
    ...
    elseif args[1] == "cancel" and ESX.PlayerData.job.name == 'racing' then
            ...
            if raceStatus.state == RACE_STATE_RACING then
                ...
                if raceStatus.checkpoint == 0 then
                    ...
                    if config_cl.checkpointRadius > 0 then
                        ...
                        checkpoint.checkpoint = CreateCheckpoint(checkpointType, checkpoint.coords.x,  checkpoint.coords.y, checkpoint.coords.z, 0, 0, 0, config_cl.checkpointRadius, 0, 191, 255, 127, 0)
                    ...
                    if GetDistanceBetweenCoords(position.x, position.y, position.z, checkpoint.coords.x, checkpoint.coords.y, 0, false) < config_cl.checkpointProximity then
                        ...
                        if raceStatus.checkpoint == #(race.checkpoints) then
                            ...
                            if config_cl.checkpointRadius > 0 then
                                local checkpointType = raceStatus.checkpoint < #race.checkpoints and RACE_CHECKPOINT_TYPE or RACE_CHECKPOINT_FINISH_TYPE
                                nextCheckpoint.checkpoint = CreateCheckpoint(checkpointType, nextCheckpoint.coords.x,  nextCheckpoint.coords.y, nextCheckpoint.coords.z, 0, 0, 0, config_cl.checkpointRadius, 0, 191, 255, 127, 0)
